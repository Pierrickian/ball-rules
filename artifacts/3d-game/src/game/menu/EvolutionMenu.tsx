...
export function EvolutionMenu({
  ...
  const sendEvolutionRequest = async () => {
    setSubmitStatus({ phase: "submitting", message: t("common.submitting") });
    try {
      ...
      setSubmitStatus({
        phase: "success",
        message: t("evolution.success", { number: typeof result.number === "number" ? ` #${result.number}` : "", title: result.title }),
        url: result.url,
      });
    } catch (error) {
      setSubmitStatus({ phase: "error", message: t("evolution.error", { message: error instanceof Error ? error.message : t("common.unknownError") }) });
    }
  };
  ...
  return (
    <div style={PANEL}>
      <div>
        <div style={TITLE}>{t("evolution.title")}</div>
        <div style={{ fontSize: 18, fontWeight: "bold", color: "#1e90ff" }}>{t("evolution.heading")}</div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button onClick={startVoiceInput} disabled={submitStatus.phase === "submitting"} style={{ ...CLOSE_BTN, flex: 1, color: "#ffe6f0", borderColor: "rgba(255,255,255,0.28)", background: voiceActive ? "rgba(255,77,122,0.34)" : "rgba(0,0,0,0.25)" }}>{voiceActive ? t("evolution.listening") : t("evolution.voice")}</button>
        <button onClick={() => void sendEvolutionRequest()} disabled={submitStatus.phase === "submitting"} style={{ ...CLOSE_BTN, flex: 1, color: "#061122", background: "#1e90ff", borderColor: "#1e90ff", fontWeight: 900 }}>{submitStatus.phase === "submitting" ? t("common.inProgress") : t("common.send")}</button>
      </div>
      {submitStatus.phase !== "idle" && (
        <div style={{ border: `1px solid ${submitStatus.phase === "error" ? "rgba(255,77,122,0.65)" : "rgba(102,255,187,0.45)"}`, background: submitStatus.phase === "error" ? "rgba(80,0,20,0.38)" : "rgba(0,60,42,0.34)", color: submitStatus.phase === "error" ? "#ffd0dc" : "#c8ffe7", borderRadius: 8, padding: "8px 10px", fontSize: 12, lineHeight: 1.4 }}>
          ...
          {submitStatus.phase === "success" && submitStatus.url && <div><a href={submitStatus.url} target="_blank" rel="noreferrer" style={{ color: "#8fd3ff" }}>{t("common.follow")}</a></div>}
        </div>
      )}
