...
const ADD_FEATURE_LOADING_MS = 2000;

const featureTypePrompt = (featureType: string) =>
  `Ajoute une fonctionnalité de ${featureType} dans l'esprit du jeu.`;
...
export const FEATURE_CATEGORIES: FeatureCategory[] = [
  {
    ...
    title: "Random",
    icon: "🎲",
    subtitle: "Let Codex choose the creative direction",
    accent: { primary: "#ffffff", secondary: "#65ffb8", gradient: "linear-gradient(135deg, #ffffff, #65ffb8, #7dd3fc, #f0abfc)", glow: "rgba(255, 255, 255, 0.6)" },
    proposals: [
      { title: "Random", subtitle: featureTypePrompt("Random"), evolutionPrompt: featureTypePrompt("Random") },
      { title: "Gameplay", subtitle: featureTypePrompt("Gameplay"), evolutionPrompt: featureTypePrompt("Gameplay") },
      { title: "Level", subtitle: featureTypePrompt("Level"), evolutionPrompt: featureTypePrompt("Level") },
      { title: "Boss", subtitle: featureTypePrompt("Boss"), evolutionPrompt: featureTypePrompt("Boss") },
      { title: "Ball", subtitle: featureTypePrompt("Ball"), evolutionPrompt: featureTypePrompt("Ball") },
      { title: "UI", subtitle: featureTypePrompt("UI"), evolutionPrompt: featureTypePrompt("UI") },
    ],
  },
  {
    id: "new_boss",
...
export function buildAddFeaturePrefill(category: FeatureCategory, proposal: ProposalAction): string {
  if (proposal.evolutionPrompt) return proposal.evolutionPrompt;
...
export function AddFeaturePortal({
  ...
  const [loadingText, setLoadingText] = useState("Loading creative options…");
  ...
  const chooseCategory = (category: FeatureCategory) => {
    ...
    setLoadingText("Loading creative options…");
  ...
  return createPortal(
    ...
    >
      ...
      <main className="add-feature-content">
        ...
        {(phase === "category_transition" || phase === "loading") && (
          <section className="add-feature-loading-stage" aria-live="polite">
            <div className="add-feature-loader" aria-hidden="true">
              <span />
              <span />
              <span />
            </div>
