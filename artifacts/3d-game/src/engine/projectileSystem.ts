...
export function processPendingCommands(engine: PlayerProjectileApiContext): void {
  ...
  for (const command of commands) {
    ...
    if (command.type === "place_mine") {
      ...
      mine.metadata = { isProjectile: true, isMine: true, lifetime: 0, fuseSeconds: 1.5, damagedIds: new Map<string, number>(), colorTint: '#ff4d7a', effect: command.effect };
...
export function handlePlayerProjectile(this: void, ball: Ball, delta: number, ctx: RuleContext): void {
  ...
  if (metaAny.isMine === true) {
    metaAny.lifetime = Number(metaAny.lifetime ?? 0) + delta;
    const fuse = typeof metaAny.fuseSeconds === "number" ? metaAny.fuseSeconds : 1.5;
    if (Number(metaAny.lifetime) >= fuse) explodeMine({
