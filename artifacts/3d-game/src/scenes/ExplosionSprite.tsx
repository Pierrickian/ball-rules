...
export type ExplosionKind = "ball" | "grenade" | "mine" | "deflagration";
...
export function ExplosionSprite({ kind, effect, t, baseBallDiameter = 1, debugTexture = false }: { kind: ExplosionKind; effect: string; t: number; baseBallDiameter?: number; debugTexture?: boolean }) {
  ...
  const grenadeScaleMultiplier = 4;
  const scale = kind === "ball"
    ? 14.4 * (0.75 + t * 1.35)
    : kind === "deflagration"
      ? baseBallDiameter * 120 * grenadeScaleMultiplier
      : kind === "mine"
        ? baseBallDiameter * 10 * grenadeScaleMultiplier
